...
import { Music2, Upload, Link2, Loader2, X } from 'lucide-react';
...
export const FileUploadMock = ({ value, onChange, label = 'Image', aspect = 'aspect-video', allowUrl = false }) => {
  const inputRef = useRef(null);
  const [loading, setLoading] = useState(false);
  const [url, setUrl] = useState('');
  const [error, setError] = useState('');
  const isVideo = label.toLowerCase().includes('video');
  const isAudio = label.toLowerCase().includes('audio') || label.toLowerCase().includes('music');
  ...
  const handleFile = async (file) => {
    ...
    setLoading(true);
    try {
      const res = await filesService.uploadAssetFile(file, label);
    ...
    <div className="space-y-2">
      ...
      >
        {value ? (
          <>
            {isVideo ? (
              <video src={value} className="h-full w-full object-cover" muted loop playsInline />
            ) : isAudio ? (
          ...
          <button type="button" disabled={loading} onClick={() => inputRef.current?.click()} className="flex flex-col items-center gap-2 p-4 text-muted-foreground disabled:cursor-wait disabled:opacity-70">
            ...
            <span className="text-xs">Compressed upload · local storage</span>
          </button>
        )}
      </div>
