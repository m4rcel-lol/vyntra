...
import { Menu, Search, Bell, ExternalLink, Eye, MousePointerClick, Sparkles } from 'lucide-react';
  ...
  const notifications = useMemo(() => {
    ...
    return [
      ...
      ...recentViews.map((view) => ({
        ...
        body: view.referrer || view.country || view.device || 'Direct traffic',
        time: view.createdAt,
      })),
      ...recentClicks.map((click) => ({
        ...
        title: 'Link clicked',
        body: click.link?.title || 'A profile link',
          ...
          {notifications.length ? (
            <div className="max-h-96 overflow-y-auto p-1">
              {notifications.map((item) => (
                <div key={item.id} className="flex gap-3 rounded-md px-2 py-2.5 text-sm hover:bg-accent">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">{formatRelative(item.time)}</p>
                  </div>
                </div>
              ))}
            </div>
