...
export async function registerSupportRoutes(app) {
    ...
    app.get("/api/support/conversations/:id", async (request) => {
        ...
        if (!conversation)
            fail(404, "SUPPORT_CONVERSATION_NOT_FOUND", "Support conversation was not found");
    ...
    app.post("/api/support/conversations/:id/messages", async (request) => {
        ...
        if (conversation.status === "CLOSED")
            fail(400, "SUPPORT_CLOSED", "This support conversation is closed");
...
async function escalateConversation(app, conversationId, requesterUserId) {
    const updated = await app.prisma.supportConversation.update({
...
async function notifyStaffQueue(app, conversationId) {
    const staff = await app.prisma.user.findMany({
        where: { isBanned: false, role: { in: ["OWNER", "ADMIN", "MODERATOR"] } },
        select: { id: true }
    });
    await Promise.all(staff.map((user) => createNotification(app, {
        userId: user.id,
        type: "support.waiting",
...
function wantsStaff(body) {
    return /\b(staff|human|representative|admin|moderator|person)\b/i.test(body);
}
function assertConversationAccess(conversation, userId, role) {
    if (conversation.requesterUserId === userId || conversation.assignedStaffId === userId || isStaffRole(role))
        return;
    fail(403, "FORBIDDEN", "You cannot access this support conversation");
}
function serializeSupportConversation(request, conversation) {
    return {
        id: conversation.id,
...
function serializeSupportMessage(request, message) {
    return {
        id: message.id,
...
function serializeSupportUser(request, user) {
