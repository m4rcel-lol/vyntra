...
export async function registerBlogRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch("/api/blog/:id", async (request) => {
    ...
    if (!existing) fail(404, "BLOG_POST_NOT_FOUND", "Blog post was not found");

    const updateData: Prisma.BlogPostUncheckedUpdateInput = {};
    if (body.title !== undefined) updateData.title = body.title;
    if (body.slug !== undefined) updateData.slug = await uniqueBlogSlug(app, body.slug, existing.id);
  ...
  app.delete("/api/blog/:id", async (request) => {
    requireRole(request, ["ADMIN"]);
    const params = z.object({ id: z.string().cuid() }).parse(request.params);
    await app.prisma.blogPost.delete({ where: { id: params.id } }).catch((error: unknown) => {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
        fail(404, "BLOG_POST_NOT_FOUND", "Blog post was not found");
      }
      throw error;
  ...
  app.post("/api/blog/:id/like", async (request) => {
    ...
    if (!post || !post.isPublished) fail(404, "BLOG_POST_NOT_FOUND", "Blog post was not found");
...
function readablePostWhere(request: FastifyRequest): Prisma.BlogPostWhereInput {
  return canManageBlog(request) ? {} : { isPublished: true };
}

function canManageBlog(request: FastifyRequest): boolean {
      ...
      select: {
        ...
        profile: {
          select: {
            ...
            avatarFileId: true,
            files: {
              where: { deletedAt: null, kind: "AVATAR" as const }
            }
          }
        }
      }
...
type BlogPostPayload = Prisma.BlogPostGetPayload<{ include: ReturnType<typeof blogPostInclude> }>;
...
async function likedPostIds(app: FastifyInstance, request: FastifyRequest, postIds: string[]): Promise<Set<string>> {
  if (!request.currentUser || postIds.length === 0) return new Set();
...
async function uniqueBlogSlug(app: FastifyInstance, input: string, ignoreId?: string): Promise<string> {
