...
export async function registerReportRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/reports", async (request) => {
    ...
    if (request.currentUser?.id) data.reporterUserId = request.currentUser.id;
...
async function assertTargetExists(app: FastifyInstance, targetType: ReportTargetType, targetId: string): Promise<void> {
