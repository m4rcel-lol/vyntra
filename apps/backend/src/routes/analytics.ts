...
const rangeDays: Record<z.infer<typeof rangeQuerySchema>["range"], number> = {
  "7d": 7,
  "30d": 30,
  "90d": 90
};
...
export async function registerAnalyticsRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/analytics/summary", async (request) => {
    ...
    const [viewsOverTime, visitorsOverTime, clicksOverTime, templateImportsOverTime] = await Promise.all([
      app.prisma.$queryRaw<Array<{ day: Date; views: bigint }>>`
        ...
        ORDER BY day ASC
      `,
      app.prisma.$queryRaw<Array<{ day: Date; visitors: bigint }>>`
        ...
        ORDER BY day ASC
      `,
      app.prisma.$queryRaw<Array<{ day: Date; clicks: bigint }>>`
        ...
        ORDER BY day ASC
      `,
      app.prisma.$queryRaw<Array<{ day: Date; imports: bigint }>>`
...
function getCount(value: number | Record<string, number>, key: string): number {
