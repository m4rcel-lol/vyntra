...
export async function registerSocialRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/friends/me", async (request) => {
    ...
    return {
      friends: friendships.map((friendship) =>
        serializePublicUser(request, friendship.requesterId === user.id ? friendship.addressee : friendship.requester)
      ),
      incomingCount,
      outgoingCount
    };
  ...
  app.get("/api/users/:username/friends", async (request) => {
    ...
    return {
      ...
      friends: friendships.map((friendship) =>
        serializePublicUser(request, friendship.requesterId === target.id ? friendship.addressee : friendship.requester)
      )
    };
  });
  ...
  app.post("/api/users/:username/friend", async (request) => {
    ...
    const target = await findUserByUsername(app, params.username);
    if (!target) fail(404, "USER_NOT_FOUND", "User was not found");
    if (target.id === actor.id) fail(400, "SELF_FRIEND_REQUEST", "You cannot add yourself as a friend");
  ...
  app.delete("/api/users/:username/friend", async (request) => {
    ...
    const target = await findUserByUsername(app, params.username);
    if (!target) fail(404, "USER_NOT_FOUND", "User was not found");
    if (target.id === actor.id) fail(400, "SELF_FRIEND_REQUEST", "You cannot remove yourself as a friend");
  ...
  app.get("/api/messages/conversations/:id", async (request) => {
    ...
    if (!conversation) fail(404, "CONVERSATION_NOT_FOUND", "Conversation was not found");
  ...
  app.post("/api/messages/:username", async (request) => {
    ...
    const target = await findUserByUsername(app, params.username);
    if (!target) fail(404, "USER_NOT_FOUND", "User was not found");
    if (target.id === actor.id) fail(400, "SELF_MESSAGE", "You cannot message yourself");

    const pair = orderedPair(actor.id, target.id);
    const accepted = await app.prisma.friendship.count({ where: { pairKey: pair.key, status: "ACCEPTED" } });
    if (accepted !== 1) fail(403, "FRIEND_REQUIRED", "You can only message accepted friends");
...
async function findUserByUsername(app: FastifyInstance, username: string): Promise<PublicUserPayload | null> {
...
async function friendshipState(app: FastifyInstance, actorId: string | null, targetId: string): Promise<string> {
  ...
  if (friendship.requesterId === actorId) return "pending_sent";
  if (friendship.addresseeId === actorId) return "pending_received";
  return "none";
}

function orderedPair(userAId: string, userBId: string): { a: string; b: string; key: string } {
  const a = userAId < userBId ? userAId : userBId;
  const b = userAId < userBId ? userBId : userAId;
  return { a, b, key: `${a}:${b}` };
}

function serializePublicUser(request: FastifyRequest, user: PublicUserPayload) {
